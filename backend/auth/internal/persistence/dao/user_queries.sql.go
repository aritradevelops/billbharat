// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.29.0
// source: user_queries.sql

package dao

import (
	"context"

	"github.com/google/uuid"
)

const activateUser = `-- name: ActivateUser :one
UPDATE "users" SET deactivated_at = NULL AND updated_by = $2 WHERE id = $1 AND deactivated_at IS NOT NULL AND deleted_at IS NULL RETURNING id, human_id, name, email, dp, email_verified, phone, phone_verified, created_at, created_by, updated_at, updated_by, deactivated_at, deactivated_by, deleted_at, deleted_by
`

type ActivateUserParams struct {
	ID        uuid.UUID  `json:"id"`
	UpdatedBy *uuid.UUID `json:"updated_by"`
}

func (q *Queries) ActivateUser(ctx context.Context, arg ActivateUserParams) (User, error) {
	row := q.db.QueryRow(ctx, activateUser, arg.ID, arg.UpdatedBy)
	var i User
	err := row.Scan(
		&i.ID,
		&i.HumanID,
		&i.Name,
		&i.Email,
		&i.Dp,
		&i.EmailVerified,
		&i.Phone,
		&i.PhoneVerified,
		&i.CreatedAt,
		&i.CreatedBy,
		&i.UpdatedAt,
		&i.UpdatedBy,
		&i.DeactivatedAt,
		&i.DeactivatedBy,
		&i.DeletedAt,
		&i.DeletedBy,
	)
	return i, err
}

const createUser = `-- name: CreateUser :one
INSERT INTO "users" (
   human_id, name, email, email_verified,  phone, created_by
) VALUES (
   $1, $2, $3, $4, $5, $6
) RETURNING id, human_id, name, email, dp, email_verified, phone, phone_verified, created_at, created_by, updated_at, updated_by, deactivated_at, deactivated_by, deleted_at, deleted_by
`

type CreateUserParams struct {
	HumanID       string    `json:"human_id"`
	Name          string    `json:"name"`
	Email         string    `json:"email"`
	EmailVerified bool      `json:"email_verified"`
	Phone         string    `json:"phone"`
	CreatedBy     uuid.UUID `json:"created_by"`
}

func (q *Queries) CreateUser(ctx context.Context, arg CreateUserParams) (User, error) {
	row := q.db.QueryRow(ctx, createUser,
		arg.HumanID,
		arg.Name,
		arg.Email,
		arg.EmailVerified,
		arg.Phone,
		arg.CreatedBy,
	)
	var i User
	err := row.Scan(
		&i.ID,
		&i.HumanID,
		&i.Name,
		&i.Email,
		&i.Dp,
		&i.EmailVerified,
		&i.Phone,
		&i.PhoneVerified,
		&i.CreatedAt,
		&i.CreatedBy,
		&i.UpdatedAt,
		&i.UpdatedBy,
		&i.DeactivatedAt,
		&i.DeactivatedBy,
		&i.DeletedAt,
		&i.DeletedBy,
	)
	return i, err
}

const deactivateUser = `-- name: DeactivateUser :one
UPDATE "users" SET deactivated_at = CURRENT_TIMESTAMP AND deactivated_by = $2 WHERE id = $1 AND deactivated_at IS NULL AND deleted_at IS NULL RETURNING id, human_id, name, email, dp, email_verified, phone, phone_verified, created_at, created_by, updated_at, updated_by, deactivated_at, deactivated_by, deleted_at, deleted_by
`

type DeactivateUserParams struct {
	ID            uuid.UUID  `json:"id"`
	DeactivatedBy *uuid.UUID `json:"deactivated_by"`
}

func (q *Queries) DeactivateUser(ctx context.Context, arg DeactivateUserParams) (User, error) {
	row := q.db.QueryRow(ctx, deactivateUser, arg.ID, arg.DeactivatedBy)
	var i User
	err := row.Scan(
		&i.ID,
		&i.HumanID,
		&i.Name,
		&i.Email,
		&i.Dp,
		&i.EmailVerified,
		&i.Phone,
		&i.PhoneVerified,
		&i.CreatedAt,
		&i.CreatedBy,
		&i.UpdatedAt,
		&i.UpdatedBy,
		&i.DeactivatedAt,
		&i.DeactivatedBy,
		&i.DeletedAt,
		&i.DeletedBy,
	)
	return i, err
}

const deleteUser = `-- name: DeleteUser :one
UPDATE "users" SET deleted_at = CURRENT_TIMESTAMP AND deleted_by = $2 WHERE id = $1 AND deleted_at IS NULL RETURNING id, human_id, name, email, dp, email_verified, phone, phone_verified, created_at, created_by, updated_at, updated_by, deactivated_at, deactivated_by, deleted_at, deleted_by
`

type DeleteUserParams struct {
	ID        uuid.UUID  `json:"id"`
	DeletedBy *uuid.UUID `json:"deleted_by"`
}

func (q *Queries) DeleteUser(ctx context.Context, arg DeleteUserParams) (User, error) {
	row := q.db.QueryRow(ctx, deleteUser, arg.ID, arg.DeletedBy)
	var i User
	err := row.Scan(
		&i.ID,
		&i.HumanID,
		&i.Name,
		&i.Email,
		&i.Dp,
		&i.EmailVerified,
		&i.Phone,
		&i.PhoneVerified,
		&i.CreatedAt,
		&i.CreatedBy,
		&i.UpdatedAt,
		&i.UpdatedBy,
		&i.DeactivatedAt,
		&i.DeactivatedBy,
		&i.DeletedAt,
		&i.DeletedBy,
	)
	return i, err
}

const findUserByEmail = `-- name: FindUserByEmail :one
SELECT id, human_id, name, email, dp, email_verified, phone, phone_verified, created_at, created_by, updated_at, updated_by, deactivated_at, deactivated_by, deleted_at, deleted_by FROM "users" WHERE email = $1 AND deleted_at IS NULL
`

func (q *Queries) FindUserByEmail(ctx context.Context, email string) (User, error) {
	row := q.db.QueryRow(ctx, findUserByEmail, email)
	var i User
	err := row.Scan(
		&i.ID,
		&i.HumanID,
		&i.Name,
		&i.Email,
		&i.Dp,
		&i.EmailVerified,
		&i.Phone,
		&i.PhoneVerified,
		&i.CreatedAt,
		&i.CreatedBy,
		&i.UpdatedAt,
		&i.UpdatedBy,
		&i.DeactivatedAt,
		&i.DeactivatedBy,
		&i.DeletedAt,
		&i.DeletedBy,
	)
	return i, err
}

const findUserById = `-- name: FindUserById :one
SELECT id, human_id, name, email, dp, email_verified, phone, phone_verified, created_at, created_by, updated_at, updated_by, deactivated_at, deactivated_by, deleted_at, deleted_by FROM "users" WHERE id = $1 AND deleted_at IS NULL
`

func (q *Queries) FindUserById(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRow(ctx, findUserById, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.HumanID,
		&i.Name,
		&i.Email,
		&i.Dp,
		&i.EmailVerified,
		&i.Phone,
		&i.PhoneVerified,
		&i.CreatedAt,
		&i.CreatedBy,
		&i.UpdatedAt,
		&i.UpdatedBy,
		&i.DeactivatedAt,
		&i.DeactivatedBy,
		&i.DeletedAt,
		&i.DeletedBy,
	)
	return i, err
}

const setUserEmailVerified = `-- name: SetUserEmailVerified :one
UPDATE "users" SET email_verified = true WHERE id = $1 AND deleted_at IS NULL RETURNING id, human_id, name, email, dp, email_verified, phone, phone_verified, created_at, created_by, updated_at, updated_by, deactivated_at, deactivated_by, deleted_at, deleted_by
`

func (q *Queries) SetUserEmailVerified(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRow(ctx, setUserEmailVerified, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.HumanID,
		&i.Name,
		&i.Email,
		&i.Dp,
		&i.EmailVerified,
		&i.Phone,
		&i.PhoneVerified,
		&i.CreatedAt,
		&i.CreatedBy,
		&i.UpdatedAt,
		&i.UpdatedBy,
		&i.DeactivatedAt,
		&i.DeactivatedBy,
		&i.DeletedAt,
		&i.DeletedBy,
	)
	return i, err
}

const setUserPhoneVerified = `-- name: SetUserPhoneVerified :one
UPDATE "users" SET phone_verified = true WHERE id = $1 AND deleted_at IS NULL RETURNING id, human_id, name, email, dp, email_verified, phone, phone_verified, created_at, created_by, updated_at, updated_by, deactivated_at, deactivated_by, deleted_at, deleted_by
`

func (q *Queries) SetUserPhoneVerified(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRow(ctx, setUserPhoneVerified, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.HumanID,
		&i.Name,
		&i.Email,
		&i.Dp,
		&i.EmailVerified,
		&i.Phone,
		&i.PhoneVerified,
		&i.CreatedAt,
		&i.CreatedBy,
		&i.UpdatedAt,
		&i.UpdatedBy,
		&i.DeactivatedAt,
		&i.DeactivatedBy,
		&i.DeletedAt,
		&i.DeletedBy,
	)
	return i, err
}

const updateUserDP = `-- name: UpdateUserDP :one
UPDATE "users" SET dp = $2 WHERE id = $1 AND deleted_at IS NULL RETURNING id, human_id, name, email, dp, email_verified, phone, phone_verified, created_at, created_by, updated_at, updated_by, deactivated_at, deactivated_by, deleted_at, deleted_by
`

type UpdateUserDPParams struct {
	ID uuid.UUID `json:"id"`
	Dp *string   `json:"dp"`
}

func (q *Queries) UpdateUserDP(ctx context.Context, arg UpdateUserDPParams) (User, error) {
	row := q.db.QueryRow(ctx, updateUserDP, arg.ID, arg.Dp)
	var i User
	err := row.Scan(
		&i.ID,
		&i.HumanID,
		&i.Name,
		&i.Email,
		&i.Dp,
		&i.EmailVerified,
		&i.Phone,
		&i.PhoneVerified,
		&i.CreatedAt,
		&i.CreatedBy,
		&i.UpdatedAt,
		&i.UpdatedBy,
		&i.DeactivatedAt,
		&i.DeactivatedBy,
		&i.DeletedAt,
		&i.DeletedBy,
	)
	return i, err
}
